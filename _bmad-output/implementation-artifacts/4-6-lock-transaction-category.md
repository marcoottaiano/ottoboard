...
- [ ] **1.1** Create a new migration file with the current timestamp:
  ```sql
  ALTER TABLE transactions
    ADD COLUMN IF NOT EXISTS category_locked BOOLEAN NOT NULL DEFAULT FALSE;
  ```
- [ ] **1.2** Apply the migration locally (`supabase db reset` or `supabase migration up`).
- [ ] **1.3** Regenerate Supabase TypeScript types: `supabase gen types typescript --local > src/lib/supabase/types.ts` (or the equivalent command in this project — check `package.json` scripts).

### Task 2: Update Transaction TypeScript interface

**File:** `src/types/index.ts`

- [ ] **2.1** Add `category_locked: boolean` to the `Transaction` interface.
- [ ] **2.2** Check `TransactionWithCategory` and any other derived types — add the field if not inherited.

### Task 3: Add lock toggle to TransactionEditModal

**File:** `src/components/finance/TransactionEditModal.tsx`

- [ ] **3.1** Read the current `TransactionEditModal` implementation.
- [ ] **3.2** Add a toggle row in the form (near the category field):
...
- [ ] **3.3** Include `category_locked` in the update payload when the user saves the modal.
...
- [ ] **4.1** Import `Lock` icon from `lucide-react`.
- [ ] **4.2** In the transaction row render, when `transaction.category_locked === true`, show a `<Lock size={12} className="text-amber-400 inline ml-1" />` icon next to the category name.
- [ ] **4.3** The icon should be visible but not intrusive — use `inline` positioning or a flex row within the category cell.

### Task 5: Update bulk recategorize to skip locked transactions

**File:** `src/hooks/useFinanceMutations.ts`

- [ ] **5.1** Locate `useBulkRecategorizeTransactions` (added in Story 4.5).
- [ ] **5.2** Before calling the Supabase bulk update, filter the `ids` array to exclude transactions where `category_locked = true`.
...
- [ ] **5.3** After the mutation succeeds, compute the count of skipped locked transactions and show a notice:
  - If `skipped > 0`: show toast "X transazioni bloccate escluse" (use the existing toast pattern).
- [ ] **5.4** The Supabase query can also add a server-side guard as defense-in-depth:
  ```typescript
  supabase
    .from('transactions')
...
- [ ] **6.1** Check that `useUpdateTransaction` supports partial field updates (i.e., it accepts `Partial<Transaction>` and only sends the provided fields).
- [ ] **6.2** If it currently requires all fields, update the signature to accept `Partial<Pick<Transaction, 'amount' | 'type' | 'category_id' | 'description' | 'date' | 'category_locked'>>`.
