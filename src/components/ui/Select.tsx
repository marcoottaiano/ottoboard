...
export function Select({ value, onChange, options, placeholder = "Seleziona...", showPlaceholder = true, className = "", disabled = false, dropUp = false }: Props) {
      ...
      >
        ...
        {options.map((opt) => (
          <button
            ...
            className={`w-full px-3 py-2 text-sm text-left transition-colors ${opt.value === value ? "bg-white/10 text-white" : "text-gray-300 hover:bg-white/5"}`}
          >
            {opt.label}
          </button>
        ))}
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button type="button" disabled={disabled} onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between gap-2 bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-sm hover:border-white/20 focus:outline-none focus:border-white/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        <span className={`truncate ${selected ? "text-gray-300" : "text-gray-600"}`}>{selected?.label ?? placeholder}</span>
        <ChevronDown size={13} className={`text-gray-600 flex-shrink-0 transition-transform duration-150 ${open ? "rotate-180" : ""}`} />
      </button>
