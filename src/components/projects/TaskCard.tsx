...
export function TaskCard({ task, isDragging = false, onClick }: Props) {
  ...
  return (
    ...
    >
      ...
      {task.linear_identifier && <span className="absolute top-2 right-2 text-[10px] font-mono px-1.5 py-0.5 rounded bg-white/5 text-gray-600 border border-white/10">{task.linear_identifier}</span>}
      ...
      {task.assignee_name && (
        <div className="mt-2 flex justify-end">
          {task.assignee_avatar ? (
            <Image src={task.assignee_avatar} alt={task.assignee_name} title={task.assignee_name} className="w-4 h-4 rounded-full" />
          ) : (
            <span title={task.assignee_name} className="w-4 h-4 rounded-full bg-purple-500/30 text-purple-300 text-[8px] flex items-center justify-center font-medium">
