...
function DetailModal({ m, onClose, onDelete, isDeleting }: DetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => !isDeleting && onClose()} role="dialog" aria-modal="true" aria-labelledby="measurement-detail-title">
      <div ref={dialogRef} tabIndex={-1} className="ob-panel w-full max-w-sm space-y-4 p-5 outline-none" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 id="measurement-detail-title" className="text-sm font-semibold text-white">
            Sessione {m.measured_at}
          </h3>
          <button onClick={onClose} disabled={isDeleting} className="ob-icon-button size-8 disabled:opacity-30" aria-label="Chiudi dettaglio sessione">
            <X size={16} />
          </button>
        </div>
        ...
        <button onClick={() => onDelete(m.id)} disabled={isDeleting} className="flex items-center gap-2 text-xs text-red-400 hover:text-red-300 disabled:opacity-50 transition-colors">
          <Trash2 size={13} />
...
export function MeasurementHistoryTable({ measurements }: Props) {
  ...
  if (measurements.length === 0) {
    return (
      <div className="ob-panel-flat flex h-24 items-center justify-center p-4">
  ...
  return (
    <>
      <div className="ob-panel-flat overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {pageData.map((m) => {
                const sumPliche = [m.skinfold_chest, m.skinfold_abdomen, m.skinfold_thigh, m.skinfold_tricep, m.skinfold_suprailiac, m.skinfold_subscapular, m.skinfold_midaxillary].filter(Boolean).reduce((a, b) => (a ?? 0) + (b ?? 0), 0);
                return (
                  <tr
                    key={m.id}
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-right text-fitness">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/10">
            ...
            <div className="flex gap-1">
              <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} className="p-1 rounded hover:bg-white/10 disabled:opacity-30 transition-colors text-gray-400">
                <ChevronLeft size={14} />
              </button>
              <button onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} className="p-1 rounded hover:bg-white/10 disabled:opacity-30 transition-colors text-gray-400">
      ...
      {selected && <DetailModal m={selected} onClose={() => setSelected(null)} onDelete={handleDelete} isDeleting={deleteMeasurement.isPending} />}
    </>
  );
}
