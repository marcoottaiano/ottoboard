...
export function HeartRateChart() {
  ...
  return (
    <div className="ob-panel-flat h-full min-h-[260px] overflow-hidden p-5">
      <div className="mb-4">
        <p className="ob-eyebrow">Cardio</p>
        <h3 className="ob-card-title mt-2">
          FC corsa <span className="font-normal text-muted">· 90 giorni</span>
        </h3>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="h-48 flex items-center justify-center text-gray-600 text-sm">Nessuna corsa con dati cardio</div>
      ) : (
        <ResponsiveContainer width="100%" height={180}>
          <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="fcMax" name="fcMax" stroke="#ef4444" strokeWidth={1} fill="url(#fcMaxGrad)" dot={false} />
            <Area type="monotone" dataKey="fcMedia" name="fcMedia" stroke="#ff6b4a" strokeWidth={2} fill="url(#fcMediaGrad)" dot={{ r: 2, fill: "#ff6b4a" }} />
          </AreaChart>
        </ResponsiveContainer>
      )}
    </div>
