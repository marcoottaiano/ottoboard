...
export function ActivityList() {
  ...
  const after = periodDays > 0 ? toLocalDateStr(new Date(Date.now() - periodDays * 86400000)) : undefined;
  ...
  return (
    <div className="ob-panel-flat overflow-hidden p-5">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <h3 className="ob-card-title flex-1">Tutte le attività</h3>
        <Select
          value={typeFilter}
          onChange={(v) => {
      ...
      {isLoading ? (
        ...
        <div className="py-12 text-center text-gray-600 text-sm">Nessuna attività trovata</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left text-xs text-gray-600" style={{ borderColor: "var(--border)" }}>
            ...
            <tbody>
              {paginated.map((a) => (
                <tr
                  ...
                  tabIndex={0}
                  className="cursor-pointer border-b transition-colors hover:bg-white/[0.035]"
                  style={{ borderColor: "var(--border)" }}
                >
                  <td className="py-2.5 text-gray-500 whitespace-nowrap">{new Date(a.start_date).toLocaleDateString("it-IT", { day: "2-digit", month: "2-digit" })}</td>
                  <td className="py-2.5">
                    <ActivityBadge type={a.type} />
                  </td>
                  <td className="py-2.5 text-white max-w-[200px] truncate">{a.name}</td>
                  <td className="py-2.5 text-gray-300 text-right whitespace-nowrap">{a.distance ? `${(a.distance / 1000).toFixed(1)} km` : "—"}</td>
                  <td className="py-2.5 text-gray-300 text-right whitespace-nowrap">{formatDuration(a.moving_time)}</td>
                  <td className="py-2.5 text-gray-300 text-right whitespace-nowrap">{a.average_heartrate ? `${Math.round(a.average_heartrate)} bpm` : "—"}</td>
                  <td className="py-2.5 text-gray-300 text-right whitespace-nowrap">{a.distance ? formatPace(a.average_pace) : "—"}</td>
                  <td className="py-2.5 text-center">
                    <a href={`https://www.strava.com/activities/${a.id}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-gray-600 transition-colors hover:text-fitness">
