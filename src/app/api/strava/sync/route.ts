...
export async function POST(request: Request) {
  ...
  if (stravaActivities.length === 0) {
    await supabase.from("strava_tokens").update({ last_synced_at: new Date().toISOString() }).eq("user_id", user.id);
  ...
  if (upsertError) {
    await supabase.from("integration_error_logs").insert({
      service: "strava",
      error_message: `Strava Sync: Errore salvataggio attività — ${upsertError.message}`,
      error_code: "500",
    });
