...
export function useUpdateTransaction() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...updates }: UpdateTransactionInput) => {
      const supabase = createClient();
      const { data, error } = await supabase.from("transactions").update(updates).eq("id", id).select("id").maybeSingle();
      if (error) throw error;
      if (!data) {
        throw new Error("Nessuna transazione aggiornata. Verifica i permessi RLS o l'esistenza della riga.");
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["transactions"] });
...
export function useDeleteTransaction() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      if (error) throw error;
      if (!data) {
        throw new Error("Nessuna transazione eliminata. Verifica i permessi RLS o l'esistenza della riga.");
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["transactions"] });
...
export function useUpsertBudget() {
  ...
  return useMutation({
    mutationFn: async ({ month, ...rest }: UpsertBudgetInput) => {
      ...
      const { error } = await supabase.from("budgets").upsert({ ...rest, month: monthDate, user_id: user.id }, { onConflict: "user_id,category_id,month" });
      if (error) throw error;
    },
    onSuccess: (_, { month }) => {
...
export function useBulkRecategorizeTransactions() {
  ...
  return useMutation({
    mutationFn: async ({ ids, categoryId }: BulkRecategorizeInput) => {
      ...
      const { error } = await supabase.from("transactions").update({ category_id: categoryId }).in("id", ids).eq("category_locked", false);
      if (error) throw error;
    },
        ...
        if (prev) {
          queryClient.setQueryData<TransactionWithCategory[]>(
            ...
            prev.map((t) => {
              ...
              return newCategory ? { ...t, category_id: categoryId, category: newCategory } : { ...t, category_id: categoryId };
            }),
          );
        }
      }
