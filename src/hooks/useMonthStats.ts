...
export function useMonthStats(month: string) {
  ...
  const { data: allTxns, isLoading: loadingAll, isError: allError, refetch: refetchAll } = useTransactions();
  ...
  return {
    isError: currentError || allError,
    refetch: () => Promise.all([refetchCurrent(), refetchAll()]),
