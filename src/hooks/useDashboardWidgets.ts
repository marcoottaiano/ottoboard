...
export type WidgetType =
  "last-activity" | "week-stats" | "month-finance" | "total-balance" | "reminders" | "financial-goal";
...
export function useAddWidget() {
  ...
  return useMutation({
    mutationFn: async ({ type, config }: { type: WidgetType; config: WidgetConfig }) => {
      ...
      const { error } = await supabase.from("dashboard_widgets").insert({ type, position: maxPos + 1, config });
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: QUERY_KEY }),
  });
  ...
  return useMutation({
    mutationFn: async (orderedIds: string[]) => {
      ...
      const results = await Promise.all(
        orderedIds.map((id, i) => supabase.from("dashboard_widgets").update({ position: i }).eq("id", id)),
      );
...
export function useUpdateWidgetConfig() {
  ...
  return useMutation({
    mutationFn: async ({ id, config }: { id: string; config: WidgetConfig }) => {
      const supabase = createClient();
      const { error } = await supabase.from("dashboard_widgets").update({ config }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: QUERY_KEY }),
  });
